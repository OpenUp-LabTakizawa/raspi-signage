...
import { Box, Button, IconButton, ToggleButton, ToggleButtonGroup, Dialog, DialogContent, Typography } from "@mui/material";
...
//          setShowError(true);
          images.forEach((image, i) => {
            let duration = 0;
            if (type == "video") {
                let videoElement = document.getElementById('video_'+ i);
    ...
    const handleOnAddImage = (e) => {
        ...
        if (type == "image") {
            ...
            for (var i = 0; i < e.target.files.length; i++) {
                // 拡張子チェック
                if (e.target.files[i].type != 'video/mp4' && e.target.files[i].type != 'video/quicktime' && e.target.files[i].type != 'video/x-ms-wmv') {
                    ...
                    return (
                      <div key={i} style={{ position: "relative", width: "40%", margin: "0.5rem" }} >
                        ...
                        {type == "image"
                          ...
                          : (
                            <video id={id} src={URL.createObjectURL(image)}
                              style={{ width: "100%" }} alt="preview" />
                        )}
                      </div>
                    )
