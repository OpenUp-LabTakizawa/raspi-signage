...
function DashboardContent({ areaDisplay, isPass, children, title = 'defaoult title' }) {
  const [open, setOpen] = React.useState(false);
  const { isAdmin, setOrderId, uid, setUid, userName, setUserName, coverageArea, progress } = useOrderContext();
...
//  }, [uid])
  React.useEffect(() => {
    async function getContentAreaData() {
      const coverageAreaList = sessionStorage.getItem('coverageArea') ? sessionStorage.getItem('coverageArea').split(","): coverageArea;
    ...
    React.useEffect(() => {
      ...
      if (uid) setName((sessionStorage && sessionStorage.getItem('userName')) ? sessionStorage.getItem('userName') : userName);
    }, [uid, userName])
  ...
  return (
    <ThemeProvider theme={mdTheme}>
      {progress ? (
        <Backdrop className={classDrop} open={progress} style={{ zIndex: "100"}}>
        ...
        <Box sx={{ display: 'flex' }}>
                ...
                <AppBar position="absolute" open={open}>
                  ...
                  >
                    ...
                    {areaDisplay && (<FormControl variant='standard' sx={{ minWidth: 120 }}>
                      ...
                      <Select
                        ...
                        sx={{ color: "white" }}
                      >
                        {contentsObjArr.length > 0 && contentsObjArr.map((obj, i) => (
                ...
                <Drawer variant="permanent" open={open}>
                  ...
                  {(!isPass && uid) && (
                    <>
                      ...
                      <ListItemButton onClick={onClickLogout}  style={{ display: "flex", alignItems: "end" }}>
                        <ListItemIcon>
                          <LogoutIcon />
                        </ListItemIcon>
...
export default function Dashboard({ areaDisplay, isPass=false, children, title }) {
  return (
    <OrderProvider>
      <DashboardContent areaDisplay={areaDisplay} isPass={isPass} title='defalut title'>
        {children}
      </DashboardContent>
    </OrderProvider>
  );
