...
//  const management = (sessionStorage.getItem("management") === "true") ? true: false;
  ...
  return (
    <React.Fragment>
      ...
      {(management || props.isAdmin) && (<CustomlistItem onClick={() => router.push('/dashboard/AreaManagement')} text="エリア管理">
        <AddBusinessIcon />
      </CustomlistItem>
      )}
      {(management || props.isAdmin) && (
        <CustomlistItem onClick={() => router.push('/dashboard/UserAccountManagement')} text="アカウント一覧管理">
          <AccountBalanceIcon />
        </CustomlistItem>
      )}
      <CustomlistItem onClick={() => router.push('/dashboard/AccountSettingManagement')} text="アカウント詳細管理">
        <ManageAccountsIcon />
      </CustomlistItem>
    </React.Fragment>
  );
