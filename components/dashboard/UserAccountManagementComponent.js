...
import { createAccountData, updateAccountData, deleteAccountData, resetAccountPassword } from "../../utilities/setContentData";
import { getUserAccountList, getContentsDataClient } from "../../utilities/getContentDataClient";
...
function UserAccountManagementComponent() {
  ...
  const { isAdmin, uid, setCoverageArea, progress, setProgress } = useOrderContext();
  ...
  useEffect(() => {
    ...
    　}
      async function getInfoData() {
        ...
        let displayList = [];
        let areaList = [];
        setUsers(getUserList.map(item => {
          const list = contents.map((content, index) => item.coverageArea && item.coverageArea.includes(content.areaId));
          return {
  ...
  const handleDetail = (index) => {
    const list = detailDisplay.map((display, i) => (i == index) ? !display: display )
    setDetailDisplay(list);
  }
   const handleSetting = (index) => {
    const list = settingDisplay.map((display, i) => (i == index) ? !display: display )
    setUsers(users.map((item, i) => {
      if (index != i) return item;
      const areaList = contentList.map(content => userList[index].coverageArea && userList[index].coverageArea.includes(content.areaId));
      return  {
        ...userList[i],
        userNameFlg: true,
...
//        return;
//      }
      const list = area.map((item, index) => {
        if (item) {
  ...
  const onClickUpdate = async (index) => {
    try{
      setProgress(true);
      const list = users[index].coverageArea.map((item, i) => {
        if(item) {
          return contentList[i].areaId;
        }
      }).filter(item => item);
      const user = {
        userName: users[index].userNameFlg ? userList[index].userName : users[index].userName,
...
//    router.reload();
  ...
  const onChangeCheckBox = (index) => {
    ...
    const onSettingChangeCheckBox = (index, i) => {
      setUsers(users.map((item, j) => {
        return {
          ...item,
          coverageArea: (index == j) ? item.coverageArea.map((body, k) => (i == k) ? !body: body) : item.coverageArea,
        }
      }))
    }
  ...
  return (
    <>
      <Box>
        ...
        <>
        <Grid>
          {createDisplay ? <Button variant="contained" sx={{ m: 1 }} onClick={() => setCreateDisplay(false)}>閉じる</Button> : <Button variant="contained" sx={{ m: 1 }} onClick={() => setCreateDisplay(true)}>アカウント追加</Button>}
          {createDisplay && (
            <Paper sx={{ height: 1 / 5, m: 1 }} key={"key"} style={{  position: "relative" }}>
              <Grid container>
                <Grid item xs={8} container direction="column" style={{ minWidth: "550px", display: "flex",　flexDirection: "row", alignItems: "center", padding: "20px" }}>
                  <Grid item style={{ display: "flex", height: "45px", padding: "5px", justifyContent: "space-between" }}>
                    <Typography style={{ lineHeight: "35px" }}>メールアドレス（ログイン用）： </Typography>
                    <input
                      type="text"
                      style={{ width: "50%" }}
                      name={"email"}
                ...
                <Grid item xs={8} container direction="column" style={{ minWidth: "550px", display: "flex",　flexDirection: "row", alignItems: "center", padding: "20px" }}>
                  <Grid item style={{ display: "flex", height: "45px", padding: "5px", justifyContent: "space-between" }}>
                    <Typography style={{ lineHeight: "35px" }}>パスワード： </Typography>
                    <input
                      type="password"
                      style={{ width: "50%" }}
                      name={"password"}
                ...
                <Grid item xs={8} container direction="column" style={{ minWidth: "550px", display: "flex",　flexDirection: "row", alignItems: "center", padding: "20px" }}>
                  <Grid item style={{ display: "flex", height: "45px", padding: "5px", justifyContent: "space-between" }}>
                    <Typography style={{ lineHeight: "35px" }}>ユーザー名： </Typography>
                    <input
                      type="text"
                      style={{ width: "50%" }}
                      name={"userName"}
                ...
                <Grid item xs={8} container direction="column" style={{ minWidth: "550px", display: "flex",　flexDirection: "row", alignItems: "center", padding: "20px" }}>
                  <Grid item style={{ display: "flex", height: "45px", padding: "5px", justifyContent: "space-between" }}>
                    <Typography style={{ lineHeight: "35px" }}>権限： </Typography>
                    <FormControlLabel
                        label={"管理者"}
                ...
                <Grid item xs={8} container direction="column" style={{ minWidth: "550px", display: "flex",　flexDirection: "row", alignItems: "center", padding: "20px" }}>
                  <Grid item style={{ display: "flex", height: "45px", padding: "5px", justifyContent: "space-between" }}>
                    <Typography style={{ lineHeight: "35px" }}>エリア設定： </Typography>
                      ...
                      {contentList.map((content, index) => {
                        return (
                          <FormControlLabel
                            ...
                            control={<Checkbox name={"area[" + index + "]"} checked={area[index]} onChange={(e) => onChangeCheckBox(index)} style={{ width: "20px" }}/>}
              ...
              <Button disabled={email == "" || userName == "" || password == ""} variant="contained" sx={{ m: 1 }} onClick={onClickCreate}>追加</Button>
            </Paper>
          )}
        </Grid>
        <Box style={{ display: "flex", flexDirection: "column" }}>
          {userList.map((user, index) => {
             const areas = user.coverageArea.map((area)=> {
                ...
                <Grid container>
                  <Grid item xs={5} container direction="column" style={{ minWidth: "550px", display: "flex",　flexDirection: "row", alignItems: "center", padding: "20px" }}>
                    <Grid item style={{ display: "flex", height: "45px", padding: "5px" }}>
                      <Typography style={{ lineHeight: "35px" }}>ユーザー名： {user.userName}</Typography>
                    </Grid>
                  </Grid>
                  {!settingDisplay[index] && (detailDisplay[index]
                      ?
                      (<Button variant="contained" sx={{ m: 1 }} onClick={() => handleDetail(index)}>閉じる</Button>)
                      :
                      (<Button variant="contained" sx={{ m: 1 }} onClick={() => handleDetail(index)}>詳細</Button>)
                    )}
                  ...
                  {detailDisplay[index] && (
                    <>
                      <Grid container>
                        <Grid item xs={8} container direction="column" style={{ minWidth: "550px", display: "flex",　flexDirection: "row", alignItems: "center", padding: "20px" }}>
                          <Grid item style={{ display: "flex", height: "45px", padding: "5px" }}>
                            <Typography style={{ lineHeight: "35px" }}>権限： {user.management ? "管理者" : "利用者"}</Typography>
                          </Grid>
                        </Grid>
                        <Grid item xs={8} container direction="column" style={{ minWidth: "550px", display: "flex",　flexDirection: "row", alignItems: "center", padding: "20px" }}>
                          <Grid item style={{ display: "flex", height: "45px", padding: "5px" }}>
                            <Typography style={{ lineHeight: "35px", display: "flex", flexDirection: "row"}}>エリア設定： {areas.length > 0 ? (areas.map((area, i) => (<Grid key={"areaList_"  + i} style={{ padding: "0 10px" }}>{area}</Grid>))) : (<Grid style={{ lineHeight: "35px", padding: "0 10px" }}>設定なし</Grid>)}</Typography>
                          </Grid>
                        </Grid>
                      </Grid>
                    </>
                  )}
                  {!detailDisplay[index] && (settingDisplay[index]
                    ?
                     (<Button variant="contained" sx={{ m: 1 }} onClick={() => handleSetting(index)}>閉じる</Button>)
                      :
                       (<Button variant="contained" sx={{ m: 1 }} onClick={() => handleSetting(index)}>編集</Button>)
                  )}
                        ...
                        <Grid item xs={8} container direction="column" style={{ minWidth: "550px", display: "flex",　flexDirection: "row", alignItems: "center", padding: "20px" }}>
                          <Grid item style={{ display: "flex", height: "45px", padding: "5px", justifyContent: "space-between" }}>
                            ...
                            <input
                              ...
                              onInput={e => onSettingChangeUserName(e, index)} />
                           </Grid>
                           <FormControlLabel
                             label={"変更しない"}
                             labelPlacement="start"
                             control={<Checkbox name={"users["+ index + "].userNameFlg"} checked={users[index] && users[index].userNameFlg} onChange={(e) => onSettingUserNameCheckBox(index)} style={{ width: "20px" }}/>}
                             style={{ marginRight: "0px" }} />
                        </Grid>
                        <Grid item xs={8} container direction="column" style={{ minWidth: "550px", display: "flex",　flexDirection: "row", alignItems: "center", padding: "20px" }}>
                          <Grid item style={{ display: "flex", height: "45px", padding: "5px", justifyContent: "space-between" }}>
                            ...
                            <FormControlLabel
                              ...
                              control={<input
                                ...
                                value={"management"}
                                disabled={true}
                                checked={users[index] && users[index].management === "management"}
                                onChange={e => handleSettingOptionChange(e, index)} />}
                              style={{ marginRight: "0px", width: "75px" }} />
                            <FormControlLabel
                              label={"利用者"}
                              labelPlacement="start"
                              control={<input
                                ...
                                value={"user"}
                                disabled={true}
                                checked={users[index] && users[index].management === "user"}
                                onChange={e => handleSettingOptionChange(e, index)} />}
                              style={{ marginRight: "0px", width: "75px" }} />
                          </Grid>
                        </Grid>
                              ...
                              {contentList.map((content, i) => {
                                return (
                                  <FormControlLabel
                                    ...
                                    labelPlacement="start"
                                    key={"areaName_" + i}
                                    control={<Checkbox name={"users["+ index + "].coverageArea"} checked={users[index] && users[index].coverageArea[i]} onChange={(e) => onSettingChangeCheckBox(index, i)} style={{ width: "20px" }}/>}
                                    style={{ marginRight: "0px" }} />
                                )
                              })}
                            </Grid>
