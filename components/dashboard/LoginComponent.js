...
function LoginComponent() {
    ...
    const { setIsAdmin, uid, setUid, setUserName, setCoverageArea, setProgress } = useOrderContext();
      ...
      try{
        ...
        if (user != null) {
          ...
          if (loginFlg) {
            ...
            sessionStorage.setItem("coverageArea", user.coverageArea ? user.coverageArea.join() : "");
          } else {
      ...
      <Container component="main" maxWidth="xs">
        ...
        >
          ...
          <Box component="form" onSubmit={onSubmit} sx={{ mt: 1 }}>
            ...
            />
            <FormControlLabel
              control={<Checkbox name={"loginFlg"} checked={loginFlg} onChange={(e) => setLoginFlg(!loginFlg)} color="primary" />}
              label="ログインを維持する"
            />
            <Button type="submit" fullWidth variant="contained" sx={{ mt: 3, mb: 2 }}>ログイン</Button>
            <Grid container>
              <Grid item xs>
                <Link href="#" variant="body2" onClick={() => {setStatus({
                                                                 open: true,
                                                                 type: "error",
