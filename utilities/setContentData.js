...
import { collection, getDocs, getDoc, addDoc, doc, getFirestore, setDoc, updateDoc, query, where } from "firebase/firestore";
import { getAuth, createUserWithEmailAndPassword, EmailAuthProvider, reauthenticateWithCredential, updatePassword, fetchSignInMethodsForEmail, confirmPasswordReset, sendEmailVerification } from "firebase/auth";
...
export const setContentPixelSize = async (orderId, pixelSizeId, width, height) => {
    if (pixelSizeId == "") {
        const app = createFirebaseApp();
        const db = getFirestore(app);
        const pixelSize = {
        ...
        const content = await setDoc(doc(ref, ids[0]), {
        ...
        if(!pixelSize.getPixelFlg) {
            ...
            const content = await updateDoc(ref, {
                pixelWidth: width,
                pixelHeight: height,
                getPixelFlg: false
            }, { merge: true });
...
// 表示画面調整の項目更新
export const updateDisplayContent = async (pixelSizeId, height, width, marginTop, marginLeft) => {
    const app = createFirebaseApp();
    const db = getFirestore(app);
    const ref = doc(db, 'PixelSize', pixelSizeId);
    await updateDoc(ref, {
...
export const updateAccountData = async (uid, user, email, nowPassword, newPassword) => {
    const app = createFirebaseApp();
    const db = getFirestore(app);
    if (newPassword) {
      const auth = getAuth();
