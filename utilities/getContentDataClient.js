...
export const checkAccountPassKey = async (email) => {
    const app = createFirebaseApp();
    const db = getFirestore(app);
    const q = query(collection(db, "users"), where("delete", "==", false), where("email", "==", email), where("passFlg", "==", true));
    const contentDocs = await getDocs(q);
    if (!contentDocs) {
        return null
    }
...
export const getContentList = async (coverageAreaList) => {
    const app = createFirebaseApp();
    const db = getFirestore(app);
    const q = query(collection(db, "contents"), where("delete", "==", false), where("areaId", "in", coverageAreaList));
    const contentDocs = await getDocs(q);
    if (!contentDocs) {
        return null
    }
