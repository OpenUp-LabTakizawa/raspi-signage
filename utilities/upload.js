...
import { getFirestore, collection, query, where, getDocs, arrayUnion } from "firebase/firestore";
import { updateContentOrder } from "./setContentData";

// コンテンツ登録時のfirebase接続等処理
export const postContent = async (docId, content, type, duration, callbackfn = undefined) => {
    if (content.name) {
        const app = createFirebaseApp();
        const storage = getStorage(app);
        const db = getFirestore(app);
        const q = query(collection(db, 'contents'), where("delete", "==", false), where("orderId", "==", docId));
